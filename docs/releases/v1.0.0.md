...
|:----------|:------------|:---------|
| **Cello v1.0.0** | **1,500,000+** | **1.0x (baseline)** |
| Robyn | ~400,000 | ~3.75x slower |
| Litestar | ~50,000 | ~30x slower |
| FastAPI | ~30,000 | ~50x slower |

*Benchmarks performed with JSON serialization on a multi-core Linux server. Results vary by hardware and workload.*
...
**Metrics Optimization:** Latency tracking uses a VecDeque ring buffer for O(1) push/pop instead of O(n) `Vec::remove(0)`.
...
|:--------|:-----|:-----------|
| **v1.0.0** | 2026-02-21 | Production-ready stable release, 1.5M req/s, API stability |

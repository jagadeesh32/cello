...
| Litestar | ~15,000 | ~10x slower |
| FastAPI | ~9,000 | ~16x slower |

*Benchmarks performed with JSON serialization on a 4-core Linux server with wrk (12 threads, 400 connections). Multi-worker mode enabled. Results vary by hardware and workload.*

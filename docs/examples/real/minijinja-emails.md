---
title: MiniJinja Email Templates
description: Serving plain-text and HTML email previews from a standalone template engine using MiniJinja and Cello
---

# :material-email-outline: MiniJinja Email Templates

This example demonstrates how to deploy two standalone template engine instances (one for HTML emails with escaping, and another for raw text emails without escaping) to compile dual-format emails. It also shows how to set up preview routes to eyeball resulting emails from a web browser.

## Features Demonstrated

- **Dual Rendering Modes**: Running `MiniJinjaEngine(auto_escape=True)` alongside an instance with `auto_escape=False` for text output.
- **Inheritance Layouts**: Styling rich email layouts in `base_email.html` with tabular frames and blocks.
- **Formatter Filters**: Formatting currency dynamically inside HTML templates via `"%.2f" | format(line.amount)`.
- **Email Previews**: Mounting handlers that return `Response.html()` or `Response.text()` to render preview formats.
...
python examples/minijinja_emails.py
# Test endpoints:
curl http://127.0.0.1:8085/preview/welcome/Alice
curl http://127.0.0.1:8085/text/welcome/Alice
curl http://127.0.0.1:8085/preview/invoice/42
```

## Key Concepts

- **Direct Instantiation**: Creating `MiniJinjaEngine` outside of an `App` object for decoupled template formatting tasks (like mail merges, report generation, etc.).
- **Dual Escaping Modes**: Setting up separate engines with `auto_escape=True` for browser-renderable elements to counter injection attacks, and `auto_escape=False` for raw emails.
- **Filter Formatting**: MiniJinja's python-style string operations let you format floating numbers into exact string fractions without pre-processing the floats in handlers.
